 ...
 * - `hide_compendia`: hides all packs shipped by the `nonex-ist-od6s` system except Macros.
...
export class OD6SCompendiumDirectory extends foundry.applications.sidebar.tabs.CompendiumDirectory {
    async _prepareDirectoryContext(context: any, options: any): Promise<void> {
        ...
        const hideOd6s = game.settings.get("nonex-ist-od6s", "hide_compendia");
        const hideAdvDis = game.settings.get("nonex-ist-od6s", "hide_advantages_disadvantages");
        ...
        for (const key of Object.keys(context.packContext)) {
            ...
            if (hideOd6s && meta.packageName === "nonex-ist-od6s" && pack.documentName !== "Macro") {
