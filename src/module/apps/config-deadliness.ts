...
export default class od6sDeadlinessConfiguration extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static DEFAULT_OPTIONS = {
        ...
        tag: "form",
        window: {
            title: "NONEX_IST_OD6S.CONFIG_DEADLINESS_MENU",
            resizable: true,
            minimizable: true,
        },
        position: {
        ...
        actions: {
            closeForm: od6sDeadlinessConfiguration.#onCloseForm,
        },
    };
    ...
    async _prepareContext(_options?: object): Promise<object> {
        const settings = Array.from(game.settings.settings)
            .filter((s: any) => s[1].od6sDeadliness)
            .map((i: any) => i[1]);
    ...
    static async #onSubmit(
        this: od6sDeadlinessConfiguration,
        _event: Event,
